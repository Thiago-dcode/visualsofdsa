...
const LinearNodeComponent = forwardRef<HTMLDivElement, props>(({ node, dsType, style = {}, className = '' }: props, ref) => {
  ...
  return (
    <>
      {node && <div ref={ref} id={`${dsType}-node-${node.id}`} style={style} className={cn('relative border-4 border-white text-center flex items-center justify-center overflow-auto ', className)}>
        <p style={
          {
